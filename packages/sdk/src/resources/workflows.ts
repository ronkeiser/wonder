...
export class WorkflowsResource {
   ...
   */
  async start(input: WorkflowInput): Promise<WorkflowStartResponse> {
   ...
   * Convenience method that combines start and streamEvents
   */
  async *execute(input: WorkflowInput): AsyncGenerator<WorkflowEvent> {
    const { durable_object_id } = await this.start(input);
