...
Immutable workflow graph definition. Changes create new versions. Context schema drives DO SQLite table generation via `@wonder/schema`.
...
**Storage:** DO SQLite (Coordinator)  
**Schema:** **Dynamic** - Generated from `WorkflowDef.context_schema` via `@wonder/schema`
...
- `@wonder/schema` DMLGenerator produces INSERT/UPDATE/DELETE statements
