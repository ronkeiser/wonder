...
curl "https://wonder-http.ron-keiser.workers.dev/api/events?workflow_run_id=run_123"

# Filter by event type
curl "https://wonder-http.ron-keiser.workers.dev/api/events?workflow_run_id=run_123&event_type=node_completed"

# Filter by node
curl "https://wonder-http.ron-keiser.workers.dev/api/events?workflow_run_id=run_123&node_id=process_step"

# Filter by token (specific execution branch)
curl "https://wonder-http.ron-keiser.workers.dev/api/events?token_id=tok_abc123"

# Pagination
curl "https://wonder-http.ron-keiser.workers.dev/api/events?workflow_run_id=run_123&limit=50&after_sequence=100"
```
...
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?workflow_run_id=run_123&category=sql&min_duration_ms=50"

# Debug specific token
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?token_id=tok_abc123"

# Performance profiling
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?workspace_id=ws_123&min_duration_ms=100&limit=50"
```
...
curl "https://wonder-http.ron-keiser.workers.dev/api/logs?workspace_id=ws_123&level=error"
```
...
curl "https://wonder-http.ron-keiser.workers.dev/api/events?workflow_run_id=run_123&limit=1000" | jq '.events[] | {event_type, node_id, token_id}'

# 2. Get trace events (internal decisions)
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?workflow_run_id=run_123&category=decision" | jq '.events[] | {type, payload}'

# 3. Check for errors
curl "https://wonder-http.ron-keiser.workers.dev/api/logs?trace_id=run_123&level=error"
```
...
```bash
# Find slow SQL queries
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?workflow_run_id=run_123&category=sql&min_duration_ms=50" | jq '.events[] | {type, duration_ms, payload}'

# Find slow operations
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?workflow_run_id=run_123&min_duration_ms=100" | jq '.events[] | {type, category, duration_ms}'
```
...
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?workflow_run_id=run_123&type=decision.sync.start"

# Get branch table lifecycle
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?workflow_run_id=run_123" | jq '.events[] | select(.type | contains("branch_table"))'
```
...
curl "https://wonder-http.ron-keiser.workers.dev/api/events?token_id=tok_abc123" | jq '.events[] | {sequence_number, event_type, node_id}'

# Internal operations for token
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?token_id=tok_abc123" | jq '.events[] | {sequence, type, payload}'
```
...
curl "https://wonder-http.ron-keiser.workers.dev/api/logs?workspace_id=ws_123&level=error&limit=100"

# Find workflows with slow queries
curl "https://wonder-http.ron-keiser.workers.dev/api/events/trace?workspace_id=ws_123&category=sql&min_duration_ms=100" | jq 'group_by(.workflow_run_id) | map({workflow_run_id: .[0].workflow_run_id, slow_queries: length})'
```
