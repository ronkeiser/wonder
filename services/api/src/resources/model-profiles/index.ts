...
export class ModelProfiles extends Resource {
  async create(data: {
    ...
    this.serviceCtx.logger.info('model_profile_create_started', {
      name: data.name,
    ...
    try {
      ...
      this.serviceCtx.logger.info('model_profile_created', {
      ...
      if (dbError.constraint === 'unique') {
        this.serviceCtx.logger.warn('model_profile_create_conflict', {
          name: data.name,
          field: dbError.field,
        });
      ...
      this.serviceCtx.logger.error('model_profile_create_failed', {
        name: data.name,
  ...
  async get(id: string): Promise<{
    ...
    this.serviceCtx.logger.info('model_profile_get', { model_profile_id: id });

    const profile = await repo.getModelProfile(this.serviceCtx.db, id);
    if (!profile) {
      this.serviceCtx.logger.warn('model_profile_not_found', { model_profile_id: id });
  ...
  async list(params?: { limit?: number; provider?: ModelProvider }): Promise<{
    ...
    this.serviceCtx.logger.info('model_profile_list', params);
  ...
  async delete(id: string): Promise<{ success: boolean }> {
    this.serviceCtx.logger.info('model_profile_delete_started', { model_profile_id: id });
    ...
    this.serviceCtx.logger.info('model_profile_deleted', { model_profile_id: id });
